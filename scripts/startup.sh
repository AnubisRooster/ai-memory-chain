...
while ! "$DOCKER" info > /dev/null 2>&1; do
  ATTEMPTS=$((ATTEMPTS + 1))
  if [ $ATTEMPTS -ge 60 ]; then log "ERROR: Docker not ready. Giving up."; exit 1; fi
...
while ! curl -s http://localhost:8545 > /dev/null 2>&1; do
  ATTEMPTS=$((ATTEMPTS + 1))
  if [ $ATTEMPTS -ge 30 ]; then log "WARNING: Polygon Edge not ready."; break; fi
  sleep 2
done
...
while ! curl -s -X POST http://localhost:5001/api/v0/id > /dev/null 2>&1; do
  ATTEMPTS=$((ATTEMPTS + 1))
  if [ $ATTEMPTS -ge 15 ]; then log "WARNING: IPFS not ready."; break; fi
  sleep 2
done
log "IPFS is ready."
